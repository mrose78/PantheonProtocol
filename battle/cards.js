/*************************************************
 * SHARED MODULE
 * Data loading, parsing, and card building logic
 * used by both the Team viewer and Browse pages
 *************************************************/

export const CHARACTERS_CSV =
...
export async function loadData() {
  ...
  const characters = parseCSV(await charRes.text());
  const images     = parseCSV(await imgRes.text());
 ...
 * HELPERS
 *************************************************/
export function pickImage(images, characterId) {
  const charImgs = images.filter(i => i.character_id == characterId);
...
export function pipRow(stat, rawValue) {
...
export function threatSkulls(threatLevel) {
...
export function alignmentIcon(alignment) {
...
export function factionSymbol(universe) {
...
const IMG_ERROR_RETRY =
  `this._r=this._r||0;if(this._r++<3){setTimeout(()=>{this.src=this.src},1000*this._r)}`;

export function buildCard(c, imgUrl) {
  ...
  front.innerHTML = `
    <div class="card-art">
      <img src="${imgUrl}" alt="${c.codename}" onerror="${IMG_ERROR_RETRY}">
    </div>
    <div class="front-top">
